...
static void usage(struct arguments *args)
{
	...
	fprintf(stderr, "	-t sleep-time: "
		"sleep time for data collector [default = 0]\n");
	exit(1);
}
...
static void parse_arguments(struct arguments *args, int argc, char **argv)
{
	...
	args->name = argv[0];
	while ((c = getopt(argc, argv, "l:n:s:p:t:")) != -1)
		switch (c) {
		case 'l':
...
#endif
{
	...
	for (;;) {
		...
		if (nr < 0) {
			perror("Data Source to Data Collector link");
			return -1;
		} else if (nr == 0) {
			perror("Data Source to Data Collector link");
			break;
		...
		for (n = 0; n < (size_t)nr; n += ns) {
			...
			if (ns < 0) {
				perror("Data Collector to State Estimator link");
				return -1;
			}
#ifdef TCPR
			state->tcpr.hard.ack =
...
int main(int argc, char **argv)
{
	...
	pullsock = connect_to_peer(&pulladdr, 0);
	if (pullsock < 0) {
